...
BTC_CONFIG_PATH = os.path.join(os.getcwd(), "audiocraft", "modules", "btc", "run_config.yaml")
BTC_WEIGHTS_PATH = os.path.join(os.getcwd(), "audiocraft", "modules", "btc", "test", "btc_model_large_voca.pt")

class ChordExtractor(nn.Module):

    def __init__(self, device, sample_rate, max_duration, chroma_len, n_chroma, winhop):
        super().__init__()
        self.config = HParams.load(BTC_CONFIG_PATH) #gotta specify the path for run_config.yaml of btc

        # self.config.feature['large_voca'] = False
        # self.config.model['num_chords'] = 25

        self.model_file = BTC_WEIGHTS_PATH
