...
class BeatConditioner(WaveformConditioner):
    ...
    def __init__(self, 
        ...
        self.feature_len = self._get_num_frames()
        ...
        if cache_path is not None:
            self.cache = EmbeddingCache(Path(cache_path) / 'wav', self.device,
                                        compute_embed_fn=self._get_full_features_for_cache,
    ...
    @torch.no_grad()
    def _get_full_features_for_cache(self, path: tp.Union[str, Path], x: WavCondition, idx: int) -> torch.Tensor:
...
class ChromaStemConditioner(WaveformConditioner):
    ...
    def __init__(self, output_dim: int, 
        ...
        self.chroma_len = self._get_num_frames()
        self.eval_wavs: tp.Optional[torch.Tensor] = self._load_eval_wavs(eval_wavs, n_eval_wavs)
        self.cache = None
        if cache_path is not None:
            self.cache = EmbeddingCache(Path(cache_path) / 'wav', self.device,
...
class BeatChromaChordConditioner(ChromaStemConditioner):
    """Beat and Chord Chroma conditioner based on stems.
    The BeatChromaChordConditioner uses DEMUCS to first filter out drums and bass, as
    the drums and bass often dominate the chroma leading to the chroma features
    not containing information about the melody.

    The beat extraction is handled by BeatNet.
    ...
    def __init__(self, 
        ...
        self.frames_per_second = self.sample_rate / self.winhop

        self.chroma = ChordExtractor(device = device, sample_rate=sample_rate, n_chroma=n_chroma, max_duration = duration, chroma_len = self.num_frames, winhop = self.winhop).to(device)
        self.chords = chords.Chords()
        self.chroma_coefficient = 1

        self.beat = BeatExtractor(sample_rate=sample_rate, 
    ...
    def _compute_wav_embedding(self, wav: torch.Tensor, sample_rate: int) -> torch.Tensor:
        ...
        # force to same size by resampling chords to finer resolution
        upsampled_chroma = list()
        for n in range(chroma.shape[-1]):
            
            y = torchaudio.functional.resample(waveform=chroma[0, :, n].squeeze().float(),
                                               orig_freq=chroma.shape[1], 
                                               new_freq=beat_ramps.shape[1])
            y = y.bool()
            upsampled_chroma.append(y)

        upsampled_chroma = torch.vstack(upsampled_chroma).T.unsqueeze(0)
        features = torch.cat([upsampled_chroma, beat_ramps], dim=2)
    ...
    def set_continuation_count(self, sub_duration_ratio, current_iter):
        self.continuation_count = int(self.num_frames * sub_duration_ratio * current_iter)
    ...
    # def _str2chroma(self, chords_texts, bpm, meter) -> torch.Tensor:
        ...
        # chroma = torch.stack(chromas)*self.chroma_coefficient
        # return chroma
    def _chord_texts2chroma(self, chord_text: str, downbeat_times: tp.List[float]) -> torch.Tensor:
        """Convert text descriptions of chords into chromatic features.

        Args:
            chord_texts (str): string of harmonic tokens
            downbeat_times (tp.List[float]): list of downbeat times
        ...
        - `downbeat_times` must have one more downbeat than `chord_texts` has measures
        ...
        measures = chord_text.split(' ')
        assert len(measures) == len(downbeat_times)-1, "`downbeat_times` must have one more downbeat than `chord_texts` has measures"

        features = torch.zeros(size=[self.num_frames, len(self.chroma_indices)])

        for n, measure in enumerate(measures):
            tokens = measure.split(',')
            measure_start_time = downbeat_times[n]
            measure_end_time = downbeat_times[n+1]
            ...
            for n, token in enumerate(tokens):
                ...
                features[frames, :] = torch.Tensor(np.tile(chroma, (num_frames, 1)))
